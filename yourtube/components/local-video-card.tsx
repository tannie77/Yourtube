...
export default function LocalVideoCard({ video }: { video: LocalVideo }) {
  ...
  return (
    <Link href={`/watch/${video._id}`} className="group overflow-hidden rounded-2xl border border-[#e8ebf0] dark:border-[#3b465f] bg-white dark:bg-[#202a3d] shadow-[0_8px_22px_rgba(24,33,55,0.025)] transition hover:-translate-y-1 hover:shadow-[0_18px_35px_rgba(24,33,55,0.08)] focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-[#ed6049]">
      ...
      <div className="p-5">
        <h3 className="line-clamp-2 text-base font-semibold tracking-[-0.03em] text-[#172033] dark:text-[#e6ecf7]">{video.videotitle}</h3>
        <p className="mt-2 text-sm text-[#697486] dark:text-[#aab5c8]">{video.videochanel}</p>
        <p className="mt-3 text-xs text-[#9aa3b2] dark:text-[#aab5c8]">{locked ? `Membership needed · ${accessPlanName(video.accessPlan)} or higher` : addedDate(video.createdAt)}</p>
      </div>
    </Link>
  );
}
