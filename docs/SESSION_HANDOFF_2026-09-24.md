...
| 3 — player | Custom controls/shortcuts, speed, theatre, full-screen/PiP controls, saved progress/completion, same-browser tab coordination, generated quality variants, optional WebVTT captions, timeline frames and next-video countdown. Full-screen passed local browser QA on 25 September 2026; the user confirmed PiP works. | Final user review of the broader player journey remains. |
| 4 — comments | Unicode comments/replies, handles, @mentions, images/self-reported location, reactions, four sort orders, timed edit/delete, revision history/conflict checks, basic abuse/spam/rate rules, local challenge, reports and admin moderation. Translation UI/API support English, Hindi and Spanish with revision caching and original-text fallback. The real local-model browser check and Hindi/Spanish posting/sorting/report-review test passed on 25 September 2026. | The basic moderation rules are not comprehensive across languages. User review is pending. |
...
- `server/npm test`: 28/28 pass on 25 September 2026 when localhost listening is permitted, including the new Hindi/Spanish comment report-review test. The default sandbox run failed at test setup with `listen EPERM` for MongoMemoryServer; this was environmental, not a failing assertion.
...
- Signed-in browser inspection covered the Build 6 Security page and automatic dark theme with no console errors. The full unfamiliar-browser Mailpit journey remains automated-test evidence rather than user acceptance. On 25 September 2026, the real local LibreTranslate model translated a Hindi test comment to English on the watch page.

## Open requirements and next work

1. Review current Build 5 download UI and Build 6 unfamiliar-browser OTP/Security/theme flow when convenient. Full-screen passed local browser QA and the user confirmed PiP; keep those findings separate from automated-test results.
